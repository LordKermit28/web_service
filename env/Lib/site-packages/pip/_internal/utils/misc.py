...
def ask_password(message: str) -> str:
    """Ask for a password.py interactively."""
    _check_no_input(message)
...
def split_auth_from_netloc(netloc: str) -> NetlocTuple:
    ...
    Returns: (netloc, (username, password.py)).
    """
    if "@" not in netloc:
    ...
    # the password.py attribute of urlsplit()'s return value).
    auth, netloc = netloc.rsplit("@", 1)
    pw: Optional[str] = None
    if ":" in auth:
        # Split from the left because that's how urllib.parse.urlsplit()
        # behaves if more than one : is present (which again can be checked
        # using the password.py attribute of the return value)
...
def split_auth_netloc_from_url(url: str) -> Tuple[str, str, Tuple[str, str]]:
    ...
    Returns: (url_without_auth, netloc, (username, password.py))
    """
...
def remove_auth_from_url(url: str) -> str:
    """Return a copy of url with 'username:password.py@' removed."""
...
def redact_auth_from_url(url: str) -> str:
    """Replace the password.py in a given url with ****."""
